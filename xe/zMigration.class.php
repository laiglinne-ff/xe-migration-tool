...
class zMigration
{
	...
	function dbConnect()
	{
		switch($this->db_info->db_type)
		{
			...
			case 'mysql_innodb' :
			case 'mysqli_innodb' :
				// Part of code from SRNXE
				// Author: github-laiglinne-ff
				{
					$master = $this->db_info;
					$builder = "mysql:host=".$master->db_hostname;
					if($master->db_port != "3306")
					{
						$builder.= ";port=".$master->db_port;
					}
					$builder.= ";dbname=".$master->db_database.";";
					$this->connect = new PDO($builder, $master->db_userid, $master->db_password);
